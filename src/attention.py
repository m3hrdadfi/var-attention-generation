...
def _compute_attention(attention_mechanism, cell_output, attention_state,
    ...
    if config['distribution'] == 'dirichlet':
        c_alpha = tf.identity(context, name='c_alpha')
        c_scale = Dense(c_dim, activation=tf.nn.tanh, name='c_scale')(context)
        c_log_alpha = tf.exp(c_scale * c_alpha, name='c_log_alpha')

        c_mean = tf.add(
            tf.digamma(c_log_alpha[:, :c_dim - 1]),
            - tf.digamma(c_log_alpha[:, -1:]),
            name='c_mean'
        )
        _one = tf.cast(1, dtype=c_log_alpha.dtype)
        c_log_sigma = tf.sqrt(
            tf.add(
                tf.polygamma(_one, c_log_alpha[:, :c_dim - 1]),
                tf.polygamma(_one, c_log_alpha[:, -1:])
            ),
            name='c_log_sigma'
        )
    ...
    if config['distribution'] == 'dirichlet':
        context_sampled = c_mean + tf.scalar_mul(temperature, epsilon * c_log_sigma)
    else:
    ...
    if use_hmean:
        ...
        h_source_mean = tf.reduce_sum(attention_mechanism.values, axis=1) / tf.cast(
            tf.count_nonzero(tf.reduce_sum(attention_mechanism.values, axis=-1), axis=1, keepdims=True),
            dtype=tf.float32)

        # Calculate KL Loss for the context vector for h_mean prior
        if config['distribution'] == 'dirichlet':
            c_log_alpha = (c_log_alpha - h_source_mean)
            a0 = tf.reduce_sum(c_log_alpha, axis=-1, keepdims=True)
            c_kl_loss = tf.reduce_mean(
                tf.add(
                    tf.add(
                        tf.add(tf.lgamma(a0), - tf.reduce_sum(tf.lgamma(c_log_alpha), 1)),
                        tf.add(
                            tf.reduce_sum(c_log_alpha * tf.digamma(c_log_alpha), 1),
                            - tf.reduce_sum(c_log_alpha * tf.digamma(a0), 1)
                        )
                    ),
                    - tf.reduce_mean(tf.digamma(c_log_alpha) - tf.digamma(a0), 1)
                ),
                axis=1,
                name='c_kl_loss'
            )
            ...
            c_kl_loss = -0.5 * tf.reduce_sum(
                1.0 + 2 * c_log_sigma - (c_mean - h_source_mean) ** 2 - tf.exp(2 * c_log_sigma), 1,
                name='c_kl_loss')
    else:
    ...
    return attention, alignments, next_attention_state, c_kl_loss
...
class AttentionWrapper(rnn_cell_impl.RNNCell):
    ...
    def call(self, inputs, state):
        ...
        for i, attention_mechanism in enumerate(self._attention_mechanisms):
            attention, alignments, next_attention_state, c_kl_loss = _compute_attention(
